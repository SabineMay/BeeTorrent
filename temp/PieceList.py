...
class PieceList: 
    ...
    def __init__(self, _num_pieces, _piece_length, _blocks_per_piece, _block_length, output_file, hashes): 
        ...
        # dictionary containing bytes that we will write to a file
        self.piece_bytes = {}
    ...
    def request(self, piece_idx, block_idx, bee):
        self.blocks[piece_idx * self.blocks_per_piece + block_idx] = PieceList.Status.REQUESTED
        self.request_times[piece_idx * self.blocks_per_piece + block_idx] = (time.time(), bee)
    ...
    def set_timedout_requests_to_ready(self):
        ...
        for index in self.request_times:
            (time_, bee) = self.request_times[index]
            if (curr - time_) > TIMEOUT:
    ...
    def add_bytes(self, piece_idx, block_idx, block):
        if piece_idx in self.piece_bytes:
            self.piece_bytes[piece_idx][block_idx] = block
        else:
            num_blocks = self.blocks_per_piece
            if piece_idx == len(self.pieces) - 1:
                num_blocks = self.last_piece_num_blocks

            self.piece_bytes[piece_idx] = [b''] * num_blocks
            self.piece_bytes[piece_idx][block_idx] = block
    ...
    def attempt_resolve(self, piece_idx):
        ...
        # WITH RAM ###
        # write the whole piece to the file
        combined_bytestring = b''
        for bs in self.piece_bytes[piece_idx]:
            combined_bytestring += bs
        self.piece_bytes.pop(piece_idx, -1)

        
        self.output_file.seek((piece_idx * self.piece_length), 0)
        self.output_file.write(combined_bytestring)
        ####
    ...
    def get_needed_block_for_piece(self, piece_idx):
        if (self.is_resolved(piece_idx)):
            ...
            i = 0
            while (i < self.blocks_per_piece and (piece_idx != self.num_pieces - 1 or i < self.last_piece_num_blocks)):
    ...
    def get_pieces_peer_needs(self, bee):
        piece_idx = 0
        ret = []
        while piece_idx < self.num_pieces:
            if self.pieces[piece_idx] == PieceList.Status.RESOLVED and bee.bitlist[piece_idx] == False:
