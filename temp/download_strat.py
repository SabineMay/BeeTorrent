...
def get_rarest_list(swarm, plst, seed):
...
def get_rarest_piece_needed(bee, piece_list, rarest_list):
    for (ind, num_who_have) in rarest_list:
        if bee.bitfield[ind] and piece_list.piece_has_blocks_to_be_requested(ind):
            return ind
        
    #for (ind, num_who_have) in rarest_list:
    #    if bee.bitfield[ind] and not piece_list.is_resolved(ind):
    #        return ind
