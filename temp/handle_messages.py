...
def handle_request(bee: Bee, msg, piecelist: PieceList):
    ...
    length = int.from_bytes(msg[9:13:1], byteorder="big")
    if (bee.peer_choked):
        vlog("Request refused -- peer is choked\n")
        #vlog("Adding request to peer's queue\n")
        #bee.queue_request(Request(idx, begin, length))
    elif (piecelist.is_resolved(idx)): 
        vlog("Request answered -- peer is unchoked and we have the piece\n")
        piecelist.output_file.seek((idx * piecelist.piece_length) + begin, 0)
        send_message_tcp(construct_piece_msg(idx, begin, piecelist.output_file.read(length)), bee.sock)
    else:
        vlog("Request refused -- peer is unchoked but we don't have the piece\n")
...
def handle_piece(bee: Bee, msg, X, piecelist: PieceList):
    ...
    if (X != proper_block_length):
        vlog("Peer gave us an incorrectly-sized block")
        
    else: 
        if (piecelist.is_resolved(piece_idx)):
            vlog("Got a block for a piece that we already hash-verified")
        elif(piecelist.is_received(piece_idx, block_idx)):
            vlog("Got a block that we already received:")
            ...
            piecelist.add_bytes(piece_idx, block_idx, block)
            ...
            vlog("Got a piece that we didn't advertise for, or that has an incorrent status in piecelist")
            block = msg[9:]
...
def handle_port():
    vlog("No support for port requests -- BT-DHT not implemented") #TODO: decide if we ne to send back a "can't do this message"
